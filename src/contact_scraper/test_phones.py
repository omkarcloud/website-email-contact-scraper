# Standalone test script: python3 -m src.contact_scraper.test_phones
...
def test_order_number_not_a_phone(result):
    # An 8-digit order number parses as a "valid" DE number under the region
    # fallback; it must never be promoted to phones (uncertain bucket is gone,
    # so it simply disappears).
    for hit in result:
...
def test_tel_and_text_merge_one_key(result):
    ...
    # forms must collapse into exactly one phones entry.
    matching = [h for h in result if '4155552671' in h['key']]
...
def test_audit_precision_canary():
    ...
    record = {'url': page_obj.url, 'is_homepage': False,
              ...
              'phones': extract_phones(page_obj, 'DK')}
    out = build_output('a.dk', [record])
    assert [p['value'] for p in out['phones']] == ['+4541111075'], out['phones']
    assert 'phones_uncertain' not in out   # removed from the product


def test_text_noise_never_valid():
    # Junk digit shapes must never land in phones (with the uncertain bucket
    # gone they are simply dropped at extraction time).
    from .html_text import build_page
    ...
    for junk in ('1200 2015', '950 2017', '000123456', '336359125457',
                 '49.36.137.230', '07911 123456'):
...
def main():
    ...
    test_order_number_not_a_phone(result)
    test_tel_and_text_merge_one_key(result)
    test_extra_candidates()
    test_text_noise_never_valid()
